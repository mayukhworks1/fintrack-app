...
async def _write_sync_log(source: str, total: int, updated: int, error: str | None, duration_ms: int) -> None:
    pool = get_pool()
    if not pool:
        return
    try:
...
async def _run_one(source: str, runner) -> None:
    ...
    try:
        ...
        await _write_sync_log(source, result.get("total", 0), result.get("updated", 0), None, duration_ms)
        logger.info("%s: total=%s updated=%s (%sms)", source, result.get("total", 0), result.get("updated", 0), duration_ms)
    except Exception as exc:
        duration_ms = int((time.time() - started) * 1000)
        await _write_sync_log(source, 0, 0, str(exc)[:500], duration_ms)
