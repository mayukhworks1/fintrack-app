  ...
  function handlePrint() {
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      .sub{font-size:8.5px;color:#6b7280;margin-bottom:12px}
      h2{font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#374151;margin:12px 0 5px;border-bottom:1.5px solid #d1d5db;padding-bottom:2px}
      table{width:100%;border-collapse:collapse;table-layout:fixed}
      th{background:#f3f4f6;font-weight:600;font-size:7.5px;text-transform:uppercase;letter-spacing:.04em;padding:3px 5px;border:1px solid #e5e7eb;overflow:hidden;white-space:nowrap;text-align:right}
      ...
      .tfoot td{font-weight:700;background:#f3f4f6!important;border-top:2px solid #9ca3af}
      .stat-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:4px}
      .stat{background:#f9fafb;border:1px solid #e5e7eb;border-radius:4px;padding:5px 8px}
      .stat-label{font-size:7.5px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:#6b7280}
    ...
    <div class="stat-grid">
      ...
      <div class="stat"><div class="stat-label">TDS Deducted</div><div class="stat-value red">${INR(totals.tdsAmt)} (${PCT(totals.effectiveTdsPct)})</div></div>
    ...
    <table>
      <colgroup>
        <col style="width:15%"><col style="width:6%"><col style="width:13%"><col style="width:13%"><col style="width:11%"><col style="width:14%"><col style="width:14%"><col style="width:14%">
      </colgroup>
      <thead><tr>
        <th class="l">Month</th><th>#</th><th>Taxable</th><th>GST</th><th>TDS</th><th>Gross</th><th>Received</th><th>Outstanding</th>
      </tr></thead>
      <tbody>${monthlyRows.map(m => `<tr>
        <td class="l">${m.label}</td><td>${m.count}</td><td>${INR(m.taxable)}</td><td>${INR(m.gstAmt)}</td><td>${INR(m.tdsAmt)}</td><td>${INR(m.gross)}</td><td class="green">${INR(m.received)}</td><td class="red">${INR(m.outstanding)}</td>
      </tr>`).join('')}</tbody>
      <tfoot><tr class="tfoot">
        <td class="l">Total</td><td>${totals.count}</td><td>${INR(totals.taxable)}</td><td>${INR(totals.gstAmt)}</td><td>${INR(totals.tdsAmt)}</td><td>${INR(totals.gross)}</td><td>${INR(totals.received)}</td><td>${INR(totals.outstanding)}</td>
      </tr></tfoot>
    </table>
    ...
    <table>
      <colgroup>
        <col style="width:11%"><col style="width:7%"><col style="width:18%"><col style="width:8%">
        <col style="width:11%"><col style="width:9%"><col style="width:8%"><col style="width:11%"><col style="width:11%"><col style="width:11%">
      </colgroup>
      <thead><tr>
        <th class="l">Invoice #</th><th class="l">Date</th><th class="l">Client / Project</th><th class="l">Status</th>
        <th>Taxable</th><th>GST</th><th>TDS</th><th>Gross</th><th>Received</th><th>O/S</th>
      </tr></thead>
      <tbody>${rows.map(r => `<tr>
        <td class="l">${r.no}</td><td class="l">${r.date}</td><td class="l">${r.client || r.project}</td><td class="l">${r.status}</td>
      ...
      <tfoot><tr class="tfoot">
        <td class="l" colspan="4">Total (${rows.length})</td>
        <td>${INR(totals.taxable)}</td><td>${INR(totals.gstAmt)}</td><td>${INR(totals.tdsAmt)}</td><td>${INR(totals.gross)}</td><td>${INR(totals.received)}</td><td>${INR(totals.outstanding)}</td>
      </tr></tfoot>
    </table>
    </body></html>`
